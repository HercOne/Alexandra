#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# A Simply Way to Manage a Telegram Community
...
# Mongodb 
"""
client  = MongoClient('mongodb://*dbuser*:*dbpassword*@ds117729.mlab.com:17729/alexandra')
db      = client.alexandra
...
if my_file.is_file():
	...
	pprint('config.yaml file does not exists')
	sys.exit()
...
MESSAGES['discord']       	= config['MESSAGES']['discord']
MESSAGES['telegram']        = config['MESSAGES']['telegram']

ADMINS_JSON                 = config['MESSAGES']['admins_json']

# Rooms 
AGLD_ROOM     = -1001097223289  # AnthemGold Room
AGLD_ADMIN	  = -205211351			# AnthemGold Mod Room

ROOM_ID_TO_NAME = {
	AGLD_ROOM : 'HERCULES',
	# AGLD_ROOM : 'AnthemGold',
	AGLD_ADMIN: 'HERCULES',
}
...
def discord(bot, update):
	...
	message_id = update.message.message_id
	name = get_name(update)
	logger.info("/discord - "+name)

	if (update.message.chat.type == 'group') or (update.message.chat.type == 'supergroup'):
		msg = random.choice(MESSAGES['pmme']) % (name)
		bot.sendMessage(chat_id=chat_id,text=msg,reply_to_message_id=message_id, parse_mode="Markdown",disable_web_page_preview=1) 
	else:
		msg = MESSAGES['discord']

		timestamp = datetime.datetime.utcnow()
		info = { 'user_id': user_id, 'request': 'discord', 'timestamp': timestamp }
		db.pm_requests.insert(info)

		bot.sendSticker(chat_id=chat_id, sticker="CAADAgADmwcAAnlc4glvQt4mVdNoPAI",disable_notification=False)
		bot.sendMessage(chat_id=chat_id, text=msg,parse_mode="Markdown",disable_web_page_preview=1) 
...
def topstickers(bot,update):	
	...
	gifs = list(db.alexandra_stickers.aggregate(pipe))
	...
	bot.sendMessage(chat_id=AGLD_ROOM, text="AnthemGold most popular 3 stickers in the last 3 days are..." )
	for g in gifs:
		pprint(g)
		bot.sendMessage(chat_id=AGLD_ROOM, text="with "+str(g['total'])+" posts..")
		bot.sendSticker(chat_id=AGLD_ROOM, sticker=g['_id'], disable_notification=False)
		time.sleep(5)
	bot.sendMessage(chat_id=update.message.chat_id, text="message has been posted to "+ROOM_ID_TO_NAME[AGLD_ROOM] )
...
def topgif(bot,update):
	...
	gifs = list(db.alexandra_gifs.aggregate(pipe))

	bot.sendMessage(chat_id=AGLD_ROOM, text="AnthemGold most popular gif ever with "+str(gifs[0]['total'])+" posts is..." )
	bot.sendSticker(chat_id=AGLD_ROOM, sticker=gifs[0]['_id'], disable_notification=False)
	bot.sendMessage(chat_id=chat_id, text="message has been posted to "+ROOM_ID_TO_NAME[AGLD_ROOM] )
...
def topgifposters(bot, update):
	...
	users = list(db.alexandra_gifs.aggregate(pipe))

	msg = "I'm naming and shaming the top 5 gif posters in "+ROOM_ID_TO_NAME[AGLD_ROOM]
	...
	msg = bot.sendMessage(chat_id=AGLD_ROOM, text=msg )
	bot.forwardMessage(chat_id=chat_id, from_chat_id=AGLD_ROOM, message_id=msg.message_id)
	bot.sendMessage(chat_id=chat_id, text="message has been posted to "+ROOM_ID_TO_NAME[AGLD_ROOM] )
...
def todayinwords(bot, update):
	...
	msgs  = list(db.alexandra_textmessages.find({ 'timestamp': {'$gt': start } }, pipe ))
	...
	msg = bot.sendPhoto(chat_id=AGLD_ROOM, photo=open(PATH_WORDCLOUD,'rb'), caption="Today in a picture" )
	bot.sendMessage(chat_id=chat_id, text="Posted today in pictures to "+ROOM_ID_TO_NAME[AGLD_ROOM] )
...
def todaysusers(bot, update):
	...
	PATH_BG        = PATH+"graphics/agld_background.png"
	PATH_USERNAMES = PATH+"telegram-usernames.png"

	# Usernames
	d = os.path.dirname('__file__')

	wc = WordCloud(background_color=None, max_words=2000,colormap='BuPu',
	...
	msg = bot.sendPhoto(chat_id=AGLD_ROOM, photo=open("telegram-usernames.png",'rb'), caption="Todays Users" )
	bot.sendMessage(chat_id=chat_id, text="Posted today in pictures to "+ROOM_ID_TO_NAME[AGLD_ROOM] )
...
@restricted 
def promotediscord(bot, update):

	pprint('promotediscord...')
	...
	if len(fmsg) > 0:
		...
		for r in rooms:
			...
			message = fmsg[0]
			bot.sendSticker(chat_id=r, sticker="CAADAgADmwcAAnlc4glvQt4mVdNoPAI", disable_notification=False)
			...
			bot.sendMessage(chat_id=r, parse_mode="Markdown", text="Message me ("+BOTNAME.replace('_','\_')+") - to see details on how to connect to [discord](https://discord.gg/JQFAb3y)", disable_web_page_preview=True )
			bot.sendMessage(chat_id=chat_id, parse_mode="Markdown", text="Broadcast sent to "+ROOM_ID_TO_NAME[r])
			
	else:
		bot.sendMessage(chat_id=chat_id, text="Please include a message in quotes to broadcast")
...
def commandstats(bot, update):
	...
	reply = "*alexandra requests since the start of the month...*\n"
...
def new_chat_member(bot, update):
	...
	if (chat_id == AGLD_ROOM) :
		bot.restrict_chat_member(AGLD_ROOM, user_id, until_date=(datetime.datetime.now() + relativedelta(minutes=15)), can_send_messages=False, can_send_media_messages=False, can_send_other_messages=False, can_add_web_page_previews=False)
...
def echo(bot, update):
	...
	if username != None:
		...
		db.alexandra_textmessages.insert(info)
...
def sticker_message(bot, update):
	...
	if chat_id: 
		...
		if username != None:
			...
			db.alexandra_stickers.insert(info)
...
def links_and_hashtag_messages(bot, update):
	...
	if (len(find_shill) > 0) and chat_id in ROOM_ID_TO_NAME:
		...
		reply = 'AnthemGold does not allow users to post personal affiliate links. This prevents many problems that can poison a community. ' 
		...
		bot.sendMessage(chat_id=AGLD_ADMIN, text=name+" from "+ROOM_ID_TO_NAME[chat_id]+" posted a shill link",parse_mode="Markdown",disable_web_page_preview=1)

		# Forward the offending message to the mod room
		bot.forwardMessage(chat_id=AGLD_ADMIN, from_chat_id=chat_id, message_id=message_id)
...
dp.add_handler(CommandHandler('promotediscord', promotediscord))
...
# Someone private messages alexandra
...
# Shoutout to @whalepoolbtc for the help
